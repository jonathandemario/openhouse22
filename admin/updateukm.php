...
$id = $_GET["id"];
$mhs = query("SELECT * FROM ukm WHERE id = $id")[0];
if(isset($_POST["submit"])){
    if(updateukm($_POST)>0){
        echo "
        <script>
            alert ('Data berhasil diubah!!');
            document.location.href = 'ukm.php';    
        </script>";
    }else {
        echo "
        <script>
...
<body>
    ...
    <div class="container">
        <div class="text-center mb-4">
            <h3>UPDATE UKM</h3>
            <p class="text-muted">Complete the form below to update an UKM</p>
        </div>
    </div>
    ...
    <div class="container d-flex justify-content-center">
        ...
        <input type="hidden" name="logoLama" value="<?=$mhs["logo"]?>">
            ...
            if($mhs['poster'] !== ' ' && $mhs['poster'] !== '') : ?>
                <img class = mb-3 src="../asset/posterUKM/<?=$mhs['poster'];?>" width = 100px> <br>
            <?php endif;?>
            ...
            <div class="mb-3">
                ...
                <img class = mb-3 src="../asset/logoUKM/<?=$mhs['logo'];?>" width = 100px> <br>
                <input type="file" class="form-control" name="logo" >
            </div>

            <div>
                <button type="submit" class="btn btn-success" name="submit">Save</button>
                <a href="ukm.php" class="btn btn-danger">Cancel</a>
            </div>
        </form>
    </div>
