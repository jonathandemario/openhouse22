...
$ukmn = $_SESSION['ukm_admin'];
if($lk!==0){
    header("Location: lk.php");
}

$num_page = 3;
$jumlah_data = count(query("SELECT * FROM ukm"));
$jumlah_halaman = ceil($jumlah_data/$num_page);
$halamanAktif = (isset($_GET["halaman"])) ? $_GET["halaman"] : 1;

$awalData = ($num_page * $halamanAktif) - $num_page;

$ukm = query("SELECT * FROM ukm LIMIT $awalData,$num_page");
if(isset($_POST["cari"])){
    if($_POST["keyword"] === ''){
        $ukm = query("SELECT * FROM ukm LIMIT $awalData,$num_page");
    }else{
        $data = $_POST["keyword"];
        $jumlah_data = count(query("SELECT * FROM ukm WHERE 
        nama LIKE '%$data%' OR
        deskripsi LIKE '%$data%'"));
        $num_page = 5;
        $jumlah_halaman = ceil($jumlah_data/$num_page);
        ...
        $awalData = ($num_page * $halamanAktif) - $num_page;
        $ukm = query("SELECT * FROM ukm WHERE 
        nama LIKE '%$data%' OR
        ...
        <table class="table table-hover text-center">
           ...
           foreach($ukm as $row) :?>
            ...
            <tr>
                ...
                <td><?=$row["kuota"]?></td>
                <td>
                <img src="../asset/logoUKM/<?=$row["logo"]?>" width = 100px height= 100px>
                </td>
                <?php if($ukmn===0) : ?>
                <td>
                <a href="updateukm.php?id=<?=$row["id"]?>" class = "link-dark"><i class="fa-solid fa-pen-to-square fs-5 me-3"></i></a> |
                <a href="deleteukm.php?id=<?=$row["id"]?>" class = "link-dark"><i class="fa-solid fa-trash"></i></a> |
                </td>
                <?php endif;?>
